        ...
        if ( is_checkout() ) {
            if ( true === AAFW_AUTOCOMPLETE ) {
                ...
                if ( '' !== $aafw_google_api_key ) {
                    ...
                    wp_enqueue_script(
                        'aafw-googleapis',
                        'https://maps.googleapis.com/maps/api/js?key=' . $aafw_google_api_key . '&loading=async&language=' . $language . '&libraries=places&v=weekly&callback=aafw_initAutocomplete',
                        array('jquery'),
                        $this->version,
                        true
                    );
